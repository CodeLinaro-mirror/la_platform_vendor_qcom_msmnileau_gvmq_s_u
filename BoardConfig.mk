...
-include $(QCPATH)/common/msmnile_gvmq_s_u/BoardConfigVendor.mk
...
ifeq ($(TARGET_SINGLE_TREE), true)
ifeq ($(PLATFORM_VERSION) ,$(filter U 14 UpsideDownCake  W Baklava 16, $(PLATFORM_VERSION)))
    AB_OTA_PARTITIONS := vendor vbmeta vendor_dlkm system_dlkm system system_ext product boot vendor_boot
endif
else
ifeq ($(PLATFORM_VERSION) ,$(filter U 14 UpsideDownCake  W Baklava 16, $(PLATFORM_VERSION)))
    AB_OTA_PARTITIONS := vendor vbmeta vendor_dlkm system_dlkm boot vendor_boot
endif
endif
else
...
BOARD_PREBUILT_DTBOIMAGE := out/target/product/msmnile_gvmq_s_u/prebuilt_dtbo.img
...
BOARD_KERNEL_CMDLINE := debug user_debug=31 loglevel=9 print-fatal-signals=1  init=/init swiotlb=4096  kpti=0 pcie_ports=compat firmware_class.path=/vendor/firmware_mnt/image loop.max_part=7 androidboot.hardware=qcom androidboot.selinux=enforcing androidboot.memcg=1 androidboot.recover_usb=1 androidboot.load_modules_parallel=true
...
BOARD_KERNEL_CMDLINE += androidboot.console=ttyAMA0
else
